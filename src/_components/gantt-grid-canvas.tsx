...
export const GanttGridCanvas: React.FC<GanttGridCanvasProps> = ({
  dates,
  rows,
  holidays,
  columnSelection,
  ...
  const drawGrid = useCallback(() => {
    ...
    holidays.forEach((holiday) => {
      const colIndex = dates.indexOf(holiday);
      if (colIndex !== -1) {
    ...
    // 선택된 열 하이라이트
    if (columnSelection.isSelected && columnSelection.selectedColumn !== null) {
    ...
    if (monthSelection.isSelected && monthSelection.selectedMonth !== null) {
      ...
      const width =
        (monthSelection.endIndex - monthSelection.startIndex + 1) * CELL_WIDTH;
